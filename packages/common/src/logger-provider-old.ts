...
class LoggerProviderOld {
    ...
    static init(opts = defaultOpts): Options {
        ...
        LoggerProviderOld._applyPrefix()
        LoggerProviderOld._includeJsonPlugin()
     ...
     * @param loggerOptions Options returned from LoggerProviderOld.init
    ...
    static _applyPrefix(): void {
        ...
        prefix.apply(log, {
            format(level, name, timestamp) {
                return LoggerProviderOld._toText(timestamp, level, name)
            },
    ...
    static _includeJsonPlugin(): void {
        ...
        log.methodFactory = (methodName: string, logLevel: any, loggerName: string): MethodFactory => {
            ...
            return (...args: any[]): any => {
                ...
                const hasStacktrace = !!LoggerProviderOld._stacktrace()
                const needStack = hasStacktrace && this.options.stacktrace.levels.some(level => level === methodName)

                let stacktrace = needStack ? LoggerProviderOld._stacktrace() : '';
                if (stacktrace) {
                ...
                rawMethod(JSON.stringify({
                    message: LoggerProviderOld._interpolate(args),
                    level: {
                ...
                if (!escaped) {
                    ...
                    switch (flag) {
                        ...
                        case 'd':
                            ...
                            break;
                        case 'j':
                            a = LoggerProviderOld._safeStringify(arg);
                            break;
                        case 'o': {
                            let obj = LoggerProviderOld._safeStringify(arg);
                            if (obj[0] !== '{' && obj[0] !== '[') {
                                obj = `<${obj}>`;
                            }
                            a = LoggerProviderOld._constructorName(arg) + obj;
                            break;
                        }
                    }
                    return a;
