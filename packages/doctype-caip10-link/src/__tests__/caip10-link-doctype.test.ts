...
describe('Caip10LinkHandler', () => {
  ...
  it('makes genesis record correctly', async () => {
    const record = await Caip10LinkDoctype.makeGenesis({ content: undefined, metadata: COMMITS.genesis.header }, context)
    expect(record).toEqual(COMMITS.genesis)
  })
  ...
  it('throws an error if genesis record has more than one controller', async () => {
    const content: any = undefined
    const controllers = [...COMMITS.genesis.header.controllers, '0x25954ef14cebbc9af3d79876489a9cfe87043f20@eip155:1']
  ...
  it('throws an error if genesis record has controller not in CAIP-10 format', async () => {
    const content: any = undefined
    const controllers = COMMITS.genesis.header.controllers.map(address => address.split('@')[0])
  ...
  it('applies genesis record correctly', async () => {
    const state = await handler.applyCommit(COMMITS.genesis, FAKE_CID_1, context)
  ...
  it('makes signed record correctly', async () => {
    ...
    const record = await Caip10LinkDoctype._makeCommit(doctype, COMMITS.r1.desiredContent)
    expect(record).toEqual(COMMITS.r1.commit)
  })

  it('applies signed record correctly', async () => {
    let state = await handler.applyCommit(COMMITS.genesis, FAKE_CID_1, context)
    state = await handler.applyCommit(COMMITS.r1.commit, FAKE_CID_2, context, state)
    expect(state).toMatchSnapshot()
  })
  ...
  it('throws an error of the proof doesn\'t match the controller', async () => {
    ...
    await expect(handler.applyCommit(COMMITS.r1.commit, FAKE_CID_2, context, state)).rejects.toThrow(/Address doesn't match/i)
  })
  ...
  it('applies anchor commit correctly', async () => {
    ...
    state = await handler.applyCommit(COMMITS.r1.commit, FAKE_CID_2, context, state)
    // Apply anchor record
    state = await handler.applyCommit(COMMITS.r2.commit as unknown as CeramicCommit, FAKE_CID_3, context, state)
    expect(state).toMatchSnapshot()
  })
  ...
  it('Does not apply anchor record on unsupported chain', async () => {
    ...
    state = await handler.applyCommit(COMMITS.r1.commit, FAKE_CID_2, context, state)
    // Apply anchor record
    await expect(handler.applyCommit(COMMITS.r2.commit as unknown as CeramicCommit, FAKE_CID_3, context, state))
  ...
  it('Should not allow replay attack', async () => {
    ...
    state = await handler.applyCommit(commits.r2 as unknown as CeramicCommit, FAKE_CID_3, context, state)
    ...
    state = await handler.applyCommit(commits.r4 as unknown as CeramicCommit, FAKE_CID_6, context, state)
