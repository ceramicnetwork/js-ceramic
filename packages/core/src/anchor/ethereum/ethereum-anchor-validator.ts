...
export class EthereumAnchorValidator implements AnchorValidator {
  ...
  async validateChainInclusion(anchorProof: AnchorProof): Promise<void> {
    ...
    if (
      ...
      validationResult.txResponse.to !== ANCHOR_AUTHORS[this._chainId]
    ) {
      throw new Error(
        `Anchor was created using address ${
          validationResult.txResponse.to
        }. This is not the official anchoring contract address ${ANCHOR_AUTHORS[this._chainId]}`
      )
    }
  }
