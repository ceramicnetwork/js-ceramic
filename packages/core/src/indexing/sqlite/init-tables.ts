  ...
  return [
    {
      ...
      validSchema: STRUCTURES[DatabaseType.SQLITE].CONFIG_TABLE_MODEL_INDEX,
    },
    { tableName: CONFIG_TABLE_NAME, validSchema: STRUCTURES[DatabaseType.SQLITE].CONFIG_TABLE },
  ]
}
...
async function initMidTable(
  ...
  await createModelTable(DatabaseType.SQLITE, dbConnection, tableName, relationColumns)
}
...
async function _verifyMidTable(
  ...
  const expectedTableStructure = Object.assign({}, STRUCTURES[DatabaseType.SQLITE].COMMON_TABLE)
  if (modelIndexArgs.relations) {
    for (const relation of Object.keys(modelIndexArgs.relations)) {
      expectedTableStructure[relation] = STRUCTURES[DatabaseType.SQLITE].RELATION_COLUMN
    }
  }
