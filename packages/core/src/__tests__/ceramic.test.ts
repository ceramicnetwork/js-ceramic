...
describe('Ceramic integration', () => {
  ...
  it('can create Ceramic instance on default network', async () => {
    ...
    const ceramic = await Ceramic.create(ipfs1, {params: {pinsetDirectory, restoreDocuments: false}})
    await delay(1000)
  ...
  it('can create Ceramic instance explicitly on inmemory network', async () => {
    const pinsetDirectory = await tmp.tmpName()
    const ceramic = await Ceramic.create(ipfs1, { params: {networkName: 'inmemory', pinsetDirectory, restoreDocuments: false} })
    await delay(1000)
    const supportedChains = await ceramic.getSupportedChains()
    expect(supportedChains).toEqual(['inmemory:12345'])
    await ceramic.close()
  ...
  it('cannot create Ceramic instance on network not supported by our anchor service', async () => {
    const pinsetDirectory = await tmp.tmpName()
    await expect(Ceramic.create(ipfs1, { params: {networkName: 'local', pinsetDirectory, restoreDocuments: false }})).rejects.toThrow(
  ...
  it('cannot create Ceramic instance on invalid network', async () => {
    const pinsetDirectory = await tmp.tmpName()
    await expect(Ceramic.create(ipfs1, { params: {networkName: 'fakenetwork', pinsetDirectory, restoreDocuments: false }})).rejects.toThrow("Unrecognized Ceramic network name: 'fakenetwork'. Supported networks are: 'mainnet', 'testnet-clay', 'dev-unstable', 'local', 'inmemory'")
    await delay(1000)
  })
