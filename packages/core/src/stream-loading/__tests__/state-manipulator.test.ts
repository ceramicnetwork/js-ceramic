...
describeIfV3('StateManipulator test', () => {
  ...
  describe('conflict resolution', () => {
    ...
    beforeAll(async () => {
      ...
      conflictingCommits[0].cid = await dispatcher.storeEvent(conflictingCommits[0].commit, doc.id)
      conflictingCommits[1].commit.prev = conflictingCommits[0].cid
      conflictingCommits[1].cid = await dispatcher.storeEvent(conflictingCommits[1].commit, doc.id)
