...
import { RustIpfsRemoteOptions } from '@ceramicnetwork/ipfs-daemon/lib/rust-ipfs'

const seed = 'SEED'

describe('Rust Ceramic interop: core <> http-client', () => {
  jest.setTimeout(30000)
  let rustIpfs: RunningIpfs
  let ipfs: IpfsApi
  ...
  beforeAll(async () => {
    logger = new DiagnosticsLogger(LogLevel.important, false)
    rustIpfs = await createIPFSFlavor({
      name: 'rust',
      options: {
        type: 'remote',
        host: 'localhost',
