...
import { createIPFSFlavor, RunningIpfs, RustIpfsFlavor } from '@ceramicnetwork/ipfs-daemon'
...
import {
  RustIpfsEmbeddedOptions,
  RustIpfsRemoteOptions,
} from '@ceramicnetwork/ipfs-daemon/lib/rust-ipfs'

const seed = 'SEED'

describe('Ceramic interop: core <> http-client', () => {
  jest.setTimeout(30000)
  let ipfs: RunningIpfs
  ...
  beforeAll(async () => {
    // ipfs = await createIPFSFlavor({
    //   name: 'rust',
    //   options: {
    //     type: 'remote',
    //     host: 'localhost',
    //     port: 5001,
    //   } as RustIpfsRemoteOptions,
    // } as RustIpfsFlavor)
    ipfs = await createIPFSFlavor({
      name: 'rust',
      options: {
        type: 'embedded',
      } as RustIpfsEmbeddedOptions,
    } as RustIpfsFlavor)
  ...
  beforeEach(async () => {
    ...
    core = await makeCeramicCore(ipfs.api(), tmpFolder.path)
  ...
  it('healthcheck fails if ipfs unreachable', async () => {
    const isOnlineSpy = jest.spyOn(daemon.ceramic.context.ipfs, 'isOnline')
    isOnlineSpy.mockImplementation(() => {
      throw new Error('ipfs is sad now')
    })
    const res = await fetch(`http://localhost:${daemon.port}/api/v0/node/healthcheck`)
  ...
  describe('pin api', () => {
    ...
    it('force pin', async () => {
      const pinSpy = jest.spyOn(daemon.ceramic.context.ipfs.pin, 'add')
      await adminClient.admin.pin.add(docA.id)
