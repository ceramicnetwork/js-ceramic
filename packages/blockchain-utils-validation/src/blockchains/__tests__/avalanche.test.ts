...
test("invalid ethereumEOA proof should return null", async () => {
  // wrong address
  const account = new AccountID({ address: addresses[1], chainId: "eip155:43113" });
  const invalidProof = { account } as unknown as LinkProof;
  await expect(avalanche.validateLink(invalidProof)).rejects.toThrow()
  // invalid signature
  ...
  await expect(avalanche.validateLink(invalidSignatureProof)).rejects.toThrow()
});
...
test("validateLink: valid ethereumEOA proof should return proof", async () => {
  ...
  await expect(avalanche.validateLink(proof)).resolves.toEqual(proof);
});

test("validate v0 and v1 proofs", async () => {
  expect(await avalanche.validateLink(proofs.v0.valid as unknown as LinkProof)).toMatchSnapshot();
  await expect(avalanche.validateLink(proofs.v0.invalid as unknown as LinkProof)).rejects.toThrow('invalid point')
  expect(await avalanche.validateLink(proofs.v1.valid as unknown as LinkProof)).toMatchSnapshot();
  expect(await avalanche.validateLink(proofs.v1.invalid as unknown as LinkProof)).toEqual(null);
});
...
test("invalid erc1271 proof should return null", async () => {
  ...
  await expect(avalanche.validateLink(erc1271Proof)).rejects.toThrow()
});
...
test("validateLink: valid erc1271 proof should return proof", async () => {
  ...
  const authProvider = new linking.avalanche.AvalancheAuthProvider(
    provider,
    contractAddress
  );
  const proof = await authProvider.createLink('msg')
  expect(await avalanche.validateLink(proof)).toEqual(proof);
});